...
#define BORDER 2 // Pour des raisons de praticite on rajoute 2 cases autour du tableau par rapport aux dimensions donnee, ca evite de faire des if (x > 0 && x < largeur &&...)
#define INFINITY 22681 // = 9*2520 + 1 -> ne peut pas etre depasse par les quantites qui m'interesse par la suite

//J'ai essaye de rendre les noms des fonctions intuitifs quand c'etait possible donc parfois je commente pas
//Autre chose : Le code d'erreur dans ce programme est soit -1 dans les cas simples, soit quelque chose < 0
...
typedef struct point_info {
	...
	int c1; //   1/(nb minimum de cases voisines a remplir pour qu'on ait plus le choix de l'état de cette case)   
...
2 -> indeterminee
3 -> sert a rien
4 -> doit etre fixee a 0 au moment ou on fixe les mecs
...
int* cf_to_itab(struct norm_arg* all) { // Donne le tableau de cf a partir de celui des points
	int* result = malloc((all->width)*(all->height)*sizeof(int));
	int temp_i_ptab;
	int temp_i_tab;
	for (int x = 0; x < all->width; x++) {
...
//debut de quelques fonctions pour les ilist :
...
//Fin des premieres fonctions pour les ilist
...
// Quelques fonctions pour les plist :
...
// Fin des premieres fonctions pour les plist
...
int main(int argc, char* argv[]) { //Pas complique
	char* in_file;
	char* out_file;
	char* pred_file;
	int rec;
